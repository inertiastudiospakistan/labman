...
- **Admin Settings**: Implemented functional settings form that persists Laboratory Info (Name, Address, Phone, Email) to the database.
- **Dynamic Reports & Invoices**: Updated print modals to dynamically load and display the configured Laboratory Info instead of hardcoded defaults.
- **Pathologist Configuration**: Added ability to customize Pathologist Name, Qualification, Designation, and Signature text on reports via Admin Settings.

## 8. Report Designer Module
- **Admin Designer**: Implemented a visual Report Designer in Admin Settings allowing customization of:
  - **Header**: Alignment, Logo, Font Size.
  - **Patient Info**: Grid vs Inline vs Card layout.
  - **Test Table**: Compact/Standard density, Row striping, Critical result styling (Text/Highlight/Icon).
  - **Footer**: Custom text and disclaimer visibility.
- **Live Preview**: Added a real-time print preview in the designer to visualize changes immediately.
- **Theme Engine**: Integrated the design engine with the actual Report Print Modal for consistent output.
